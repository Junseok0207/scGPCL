...
from layers import GNN_Encoder
...
class scGPCL_MSE_Trainer(embedder):
    ...
    def _init_model(self):
        ...
        self.model = scGPCL_MSE(self.c_g_graph.metadata(), self.adata.n_vars, self.args.n_clusters, layers, 
    ...
    def pre_train(self):
        for epoch in tqdm(range(1, self.args.epochs+1)):
            ...
            for view1, view2 in zip(self.train_loader1, self.train_loader2):
                ...
                X = torch.tensor(self.adata.X)[sampled_id].to(self.args.device)
